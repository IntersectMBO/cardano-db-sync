...
let
  dockerImageArtifact =
    let image = (pkgsFor (builtins.head supportedSystems)).dockerImage;
  ...
  jobs = {
    ...
    cardano-db-sync-linux = import ./nix/binary-release.nix {
      ...
      platform = "linux";
      exes = filter (p: p.system == "x86_64-linux")
        (collect isDerivation jobs.musl64.exes);
    };
    cardano-db-sync-macos = import ./nix/binary-release.nix {
      inherit pkgs project;
      platform = "macos";
      exes = filter (p: p.system == "x86_64-darwin")
        (collect isDerivation jobs.native.exes);
    };
    native = let
    ...
    musl64 = mapTestOnCross musl64
      (packagePlatformsCross (filterProject noMusl64Build));
