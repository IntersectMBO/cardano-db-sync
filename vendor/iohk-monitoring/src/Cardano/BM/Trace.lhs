...
import           Cardano.BM.Data.Tracer (Tracer (..), contramap, mkTracer, natTracer,
...
\begin{code}
appendName :: Monad m => LoggerName -> Trace m a -> Trace m a
...
\begin{code}
named :: Monad m => Tracer m (LoggerName,LogObject a) -> Tracer m (LOMeta, LOContent a)
...
stdoutTrace = mkTracer $ \(ctx, LogObject _loname _ lc) ->
