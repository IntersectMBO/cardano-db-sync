...
import           Control.Tracer.Legacy (condTracing, condTracingM,
                     showTracing)
...
trFromIntegral name tr = mkTracer $ \arg ->
        traceWith tr =<< do
...
trFromReal :: (Real b, MonadIO m, HasPrivacyAnnotation b, HasSeverityAnnotation b)
           => LoggerName -> Trace m a -> Tracer m b
trFromReal name tr = mkTracer $ \arg ->
        traceWith tr =<< do
            meta <- mkLOMeta (getSeverityAnnotation arg) (getPrivacyAnnotation arg)
            return ( mempty
...
instance Transformable Text IO Text where
    trTransformer _ tr = mkTracer $ \arg ->
        traceWith tr =<< do
            meta <- mkLOMeta (getSeverityAnnotation arg) (getPrivacyAnnotation arg)
            return ( mempty
...
trStructured :: (ToObject b, MonadIO m, HasPrivacyAnnotation b, HasSeverityAnnotation b)
             => TracingVerbosity -> Trace m a -> Tracer m b
trStructured verb tr = mkTracer $ \arg ->
 let
...
trStructuredText :: ( ToObject b, MonadIO m, HasTextFormatter b
                 ...
                 => TracingVerbosity -> Trace m a -> Tracer m b
trStructuredText verb tr = mkTracer $ \arg ->
 let
   obj = toObject verb arg
 in traceWith tr =<< do
          meta <- mkLOMeta (getSeverityAnnotation arg) (getPrivacyAnnotation arg)
...
setSeverity sev tr = mkTracer $ \(ctx,lo@(LogObject _nm meta@(LOMeta _ts _tid _hn _sev _pr) _lc)) ->
...
severityDebug, severityInfo, severityNotice,
  ...
  severityAlert, severityEmergency  :: Monad m => Trace m a -> Trace m a
...
\begin{code}
annotateSeverity :: (Monad m, HasSeverityAnnotation a) => Tracer m (WithSeverity a) -> Tracer m a
annotateSeverity tr = mkTracer $ \arg ->
...
setPrivacy prannot tr = mkTracer $ \(ctx,lo@(LogObject _nm meta _lc)) ->
                                   traceWith tr $ (ctx, lo { loMeta = meta { privacy = prannot }})

annotateConfidential, annotatePublic :: Monad m => Trace m a -> Trace m a
...
\begin{code}
annotatePrivacyAnnotation :: (Monad m, HasPrivacyAnnotation a) => Tracer m (WithPrivacyAnnotation a) -> Tracer m a
annotatePrivacyAnnotation tr = mkTracer $ \arg ->
...
instance forall m a t. (Monad m, Transformable t m a) => Transformable t m (WithSeverity a) where
    trTransformer verb tr = mkTracer $ \(WithSeverity sev arg) ->
...
filterPrivacyAnnotation :: forall m a. (Monad m, HasPrivacyAnnotation a)
                        ...
                        -> Tracer m a
                        -> Tracer m a
filterPrivacyAnnotation mpa tr = mkTracer $ \arg -> do
    pa <- mpa arg
...
instance forall m a t. (Monad m, Transformable t m a) => Transformable t m (WithPrivacyAnnotation a) where
    trTransformer verb tr = mkTracer $ \(WithPrivacyAnnotation pa arg) ->
        let transformer :: Tracer m a
