...
deleteTablesAfterBlockId txOutVariantType blkId mtxId minIdsW = do
  ...
  let adaPotsStmt = deleteWhereCount @SCE.AdaPots "block_id" ">=" blockIdEncoder
      ...
      leiosCommitteeStmt = deleteWhereCount @SCB.LeiosCommittee "block_id" ">=" blockIdEncoder

  (adaPotsCount, reverseIndexCount, epochParamCount, leiosCertSignerCount, leiosCommitteeCount) <- runSession mkDbCallStack $ HsqlSes.pipeline $ do
    ...
    committee <- HsqlP.statement blkId leiosCommitteeStmt
    pure (ada, rev, epoch, leios, committee)
  ...
  let initialLogs =
        ...
        , (tableName (Proxy @SCB.LeiosCommittee), leiosCommitteeCount)
        ]
    ...
    Just vaId -> do
      ...
      logsVoting <- case mocvdId of
        ...
        Just ocvdId -> do
          ...
          (govCount, drepCount, authorCount, refCount, extCount) <- runSession mkDbCallStack $ HsqlSes.pipeline $ do
      ...
      offChain <- forM anchorDeleteOps $ \(tableN, deleteSession) -> do
        count <- runSession mkDbCallStack deleteSession
        pure (tableN, count)
  ...
  blockCount <- runSession mkDbCallStack deleteSession
...
deleteTablesAfterTxId txOutVariantType mtxId minIdsW = do
  ...
  txIdLogs <- case mtxId of
    ...
    Just txId -> do
      ...
      txCount <- runSession mkDbCallStack deleteSession
