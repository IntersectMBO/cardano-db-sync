...
import Cardano.Prelude (MonadIO (..), ask, when, MonadError (..), for_)
...
--           for_ (dbTracer dbEnv) $ \tracer -> liftIO $ logDebug tracer msg
...
runDbT :: MonadIO m => DbTransMode -> DbTransaction a -> DbAction m a
runDbT mode DbTransaction{..} = DbAction $ do
  dbEnv <- ask
  let logMsg msg =
        when (dbEnableLogging dbEnv) $
  ...
  let runSession = do
        ...
        case result of
          Left sessionErr ->
            throwError $ DbError dtCallSite "Database transaction failed: " sessionErr
          Right val -> pure val
...
data ResultType c r where
  ...
  -- WithResultKey :: HsqlD.Result (Key a) -> ResultType (Entity a) (Key a)
...
data ResultTypeBulk c r where
  ...
  -- WithResultBulkKeys :: HsqlD.Result [Key a] -> ResultTypeBulk (Entity a) [Key a]  -- Return just the keys
