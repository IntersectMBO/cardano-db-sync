...
import Cardano.Prelude hiding (map, from, isNothing, on)
...
queryDRepDistrAmount drepHash epochNo = do
 result <- runDbSession (mkDbCallStack "queryDRepDistrAmount") $
...
queryTxFeeDeposit txId = do
 result <- runDbSession (mkDbCallStack "queryTxFeeDeposit") $
...
queryTxInputs txOutTableType txId = do
 case txOutTableType of
   SV.TxOutVariantCore -> do
     cores <- runDbSession (mkDbCallStack "queryTxInputsCore") $
       HsqlSes.statement txId queryTxInputsCoreStmt
     pure $ map SV.VCTxOutW cores
   SV.TxOutVariantAddress -> do
     addresses <- runDbSession (mkDbCallStack "queryTxInputsAddress") $
...
queryTxOutputs txOutTableType txId = do
 case txOutTableType of
   SV.TxOutVariantCore -> do
     cores <- runDbSession (mkDbCallStack "queryTxOutputs TxOutVariantCore") $
       HsqlSes.statement txId queryTxOutputsCoreStmt
     pure $ map SV.VCTxOutW cores
   SV.TxOutVariantAddress -> do
     addresses <- runDbSession (mkDbCallStack "queryTxOutputs TxOutVariantAddress") $
...
queryRewardsAndRestsWithStakeAddr mEpoch = do
  res1 <- runDbSession (mkDbCallStack "queryRewardsWithStakeAddr") $
    HsqlSes.statement mEpoch queryRewardsWithStakeAddrStmt
  res2 <- runDbSession (mkDbCallStack "queryRewardRestsWithStakeAddr") $
