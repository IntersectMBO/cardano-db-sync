...
validateEpochBlockTxs = do
  mLatestEpoch <- DB.runDbMNoLoggingDefaultEnv DB.queryLatestCachedEpochNo
...
validateBlockTxs epoch = do
  ...
  blks <- DB.runDbMNoLoggingDefaultEnv $ DB.queryEpochBlockNumbers epoch
  results <- DB.runDbMNoLoggingDefaultEnv $ mapM validateBlockCount blks
