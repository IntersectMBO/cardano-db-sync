...
validateLedger params txOutVariantType =
  withIOManager $ \_ -> do
    ...
    slotNo <- SlotNo <$> DB.runDbMNoLoggingDefaultEnv DB.queryLatestSlotNo
...
validateBalance txOutVariantType slotNo addr st = do
  balanceDB <- DB.runDbMNoLoggingDefaultEnv $ DB.queryAddressBalanceAtSlot txOutVariantType addr (unSlotNo slotNo)
