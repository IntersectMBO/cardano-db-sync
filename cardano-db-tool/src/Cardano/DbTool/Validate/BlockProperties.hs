...
validateBlockProperties = do
  blkCount <- fromIntegral <$> DB.runDbMNoLoggingDefaultEnv DB.queryBlockCount
...
validateBlockTimesInPast = do
  ...
  xs <- DB.runDbMNoLoggingDefaultEnv $ DB.queryBlocksTimeAfters now
  if List.null xs
...
validataBlockNosContiguous blkCount = do
  ...
  putStrF $
    "Block numbers ["
      ...
      ++ "] are contiguous: "
  blockNos <- DB.runDbMNoLoggingDefaultEnv $ DB.queryBlockNoList startBlock testBlocks
...
validateTimestampsOrdered blkCount = do
  ...
  putStrF $
    "Block time stamps for blocks ["
      ...
      ++ show (startBlock + testBlocks)
      ++ "] are ordered: "
  ts <- DB.runDbMNoLoggingDefaultEnv $ DB.queryBlockTimestamps startBlock testBlocks
