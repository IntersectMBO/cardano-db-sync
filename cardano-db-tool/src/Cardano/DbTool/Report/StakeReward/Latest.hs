...
reportEpochStakeRewards epochNum saddr = do
  xs <- catMaybes <$> DB.runDbMNoLoggingDefaultEnv (mapM (queryEpochStakeRewards epochNum) saddr)
  renderRewards xs

reportLatestStakeRewards :: [Text] -> IO ()
reportLatestStakeRewards saddr = do
  xs <- catMaybes <$> DB.runDbMNoLoggingDefaultEnv (mapM queryLatestStakeRewards saddr)
  renderRewards xs
