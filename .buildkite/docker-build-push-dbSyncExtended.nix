# This script will load nix-built docker images of cardano-db-sync-extended applications
...
#
# 1. The repo (default "inputoutput/cardano-db-sync-extended") is changed to match
...
let
  images = mapAttrs (key: image: impureCreated image) { inherit (dbSyncPackages.dockerImages) dbSyncExtended; };
...
in
  writeScript "docker-build-push" ''
    ...
    reponame=cardano-db-sync-extended
