...
import           Cardano.Db (EntityField (..), PoolHashId, PoolMetaHash (..), PoolMetadataRef,
                   PoolOfflineData, PoolRetire, PoolUrl (..), runDbNoLoggingEnv, unValue4)

import           Cardano.DbSync.Era.Shelley.Offline.Http (FetchError (..), httpGetPoolOfflineData,
                   parsePoolUrl, renderFetchError)
...
import           Database.Esqueleto.Experimental (SqlBackend, from, innerJoin, notExists, on,
                   select, table, where_, (:&) ((:&)), (==.), (^.))
...
queryTestOfflineData = do
    res <- select $ do
            ...
            pure (pod ^. PoolOfflineDataTickerName, pmr ^. PoolMetadataRefUrl
                    , pmr ^. PoolMetadataRefHash, pod ^. PoolOfflineDataPoolId)
