...
import           Cardano.DbSync                 ( ConfigFile(..)
                                                , DbSyncNodeParams(..)
                                                , GenesisFile(..)
                                                , SocketPath(..)
                                                , runDbSyncNode
                                                )
import           Cardano.DbSync.Plugin.TxBody   ( defDbSyncTxBodyPlugin )
...
main :: IO ()
main = do
  runDbSyncNode defDbSyncTxBodyPlugin =<< Opt.execParser opts
...
opts = Opt.info
  ...
  (Opt.fullDesc <> Opt.progDesc "Cardano PostgreSQL sync node.")
...
pGenesisFile = GenesisFile <$> Opt.strOption
...
pMigrationDir = MigrationDir <$> Opt.strOption
  (  Opt.long "schema-dir"
