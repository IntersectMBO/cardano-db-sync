...
countStakePerPool (Ledger.ActiveStake activeStake) = VMap.foldlWithKey accum Map.empty activeStake
  where
    accum !acc _cred swd =
      Map.insertWith addDel (swdDelegation swd) (Ledger.fromCompact (unNonZero (swdStake swd)), 1) acc
