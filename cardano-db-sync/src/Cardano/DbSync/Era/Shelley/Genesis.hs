        ...
        Left _ ->
          runExceptT $ do
            ...
            when (hasInitialFunds || hasStakes) $ do
              ...
              lift $ mapM_ (insertTxOuts tracer hasConsumed disInOut bid) $ genesisUtxOs cfg
...
insertTxOuts trce hasConsumed disInOut blkId (TxIn txInId _, txOut) = do
...
insertStaking tracer cacheStatus blkId genesis = do
  ...
  forM_ stakes $ \(n, (keyStaking, keyPool)) -> do
    ...
    insertStakeRegistration cacheStatus (EpochNo 0) Nothing txId (2 * n) (Generic.annotateStakingCred network (KeyHashObj keyStaking))
    insertDelegation tracer cacheStatus network (EpochNo 0) 0 txId (2 * n + 1) Nothing (KeyHashObj keyStaking) keyPool
