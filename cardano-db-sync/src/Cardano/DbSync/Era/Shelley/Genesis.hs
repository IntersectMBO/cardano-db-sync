...
import Cardano.DbSync.DbEvent (liftFail, runDbSyncTransaction, runDbSyncTransactionNoLogging)
...
insertValidateShelleyGenesisDist syncEnv networkName cfg shelleyInitiation = do
  ...
  case DB.dbTracer $ envDbEnv syncEnv of
    Just trce -> ExceptT $ runDbSyncTransaction trce (envDbEnv syncEnv) (insertAction prunes)
