...
import Cardano.DbSync.Era.Universal.Insert.Other (queryOrInsertStakeRef)
        ...
        Left _ ->
          runExceptT $ do
            ...
            when (hasInitialFunds || hasStakes) $ do
              ...
              unless disInOut $ do
                lift $ mapM_ (insertTxOuts syncEnv bid) $ zip [0..] (genesisUtxOs cfg)
              liftIO . logInfo tracer $
                "Initial genesis distribution populated. Hash "
                  <> renderByteArray (configGenesisHash cfg)
              when hasStakes $ do
                let stakeTxId = DB.TxKey $ fromIntegral $ length (genesisUtxOs cfg)
                insertStaking syncEnv bid stakeTxId cfg
...
insertTxOuts ::
  ...
  SyncEnv ->
  DB.BlockId ->
  (Int, (TxIn StandardCrypto, ShelleyTxOut StandardShelley)) ->
  ReaderT SqlBackend m ()
insertTxOuts syncEnv blkId (txIndex, (TxIn txInId _, txOut)) = do
  ...
  _ <- queryOrInsertStakeRef syncEnv (txOut ^. Core.addrTxOutL)
  ...
  where
    ...
    makeVTxOut :: V.AddressId -> V.TxOut
    makeVTxOut addrDetailId =
      V.TxOut
...
insertStaking syncEnv blkId txId genesis = do
  ...
  forM_ params $ uncurry (insertPoolRegister syncEnv (const False) Nothing (EpochNo 0) blkId txId)
  ...
  forM_ stakes $ \(n, (keyStaking, keyPool)) -> do
    ...
    insertDelegation syncEnv (EpochNo 0) 0 txId (2 * n + 1) Nothing (KeyHashObj keyStaking) keyPool
