...
adjustEpochRewards syncEnv nw epochNo rwds creds = do
  ...
  crds <- rights <$> forM (Set.toList creds) (queryStakeAddrWithCache (envCache syncEnv) DontCacheNew nw)
...
deleteReward syncEnv nw epochNo (cred, rwd) = do
  ...
  eiPoolId <- queryPoolKeyWithCache syncEnv DontCacheNew (Generic.rewardPool rwd)
