...
import Cardano.DbSync.AppT (App, InsertOptions (..), MonadAppDB (..), SyncEnv (..), SyncOptions (..), askInsertOptions, askTrace)
...
import Cardano.DbSync.Cache.Types (CacheStatus (..), UpdateCache (..), EpochBlockDiff (..))
...
insertBlockUniversal shouldLog withinTwoMins withinHalfHour blk details isMember applyResult = do
  iopts <- askInsertOptions
  pbid <- case Generic.blkPreviousHash blk of
    Nothing -> liftLookupFail (renderErrorMessage (Generic.blkEra blk)) $ dbQueryToApp DB.queryGenesis -- this is for networks that fork from Byron on epoch 0.
    Just pHash -> queryPrevBlockWithCache (renderErrorMessage (Generic.blkEra blk)) pHash
  mPhid <- queryPoolKeyWithCache UpdateCache $ coerceKeyRole $ Generic.blkSlotLeader blk
  let epochNo = sdEpochNo details

  slid <- dbQueryToApp . DB.insertSlotLeader $ Generic.mkSlotLeader (ioShelley iopts) (Generic.unKeyHashRaw $ Generic.blkSlotLeader blk) (eitherToMaybe mPhid)
  blkId <-
    insertBlockAndCache $
      DB.Block
  ...
  let txInserter = insertTx isMember blkId (sdEpochNo details) (Generic.blkSlotNo blk) applyResult
  blockGroupedData <- foldM (\gp (idx, tx) -> txInserter idx tx gp) mempty zippedTx
  minIds <- insertBlockGroupedData blockGroupedData
  offChainPoolResultQueue <- asks envOffChainPoolResultQueue
  offChainVoteResultQueue <- asks envOffChainVoteResultQueue
  tracer <- askTrace
  syncOptions <- asks envOptions
  ...
  when (soptEpochAndCacheEnabled syncOptions) $
    void $
  ...
  whenStrictJust (apNewEpoch applyResult) $ \newEpoch -> do
    insertOnNewEpoch iopts blkId (Generic.blkSlotNo blk) epochNo newEpoch

  insertStakeSlice $ apStakeSlice applyResult

  when (ioGov iopts && (withinHalfHour || unBlockNo (Generic.blkBlockNo blk) `mod` 10000 == 0)) $
    insertOffChainVoteResults offChainVoteResultQueue

  when (ioOffChainPoolData iopts && (withinHalfHour || unBlockNo (Generic.blkBlockNo blk) `mod` 10000 == 0)) $
    insertOffChainPoolResults offChainPoolResultQueue
  pure $ Right ()
  where
