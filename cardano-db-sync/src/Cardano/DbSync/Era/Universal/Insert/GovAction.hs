...
insertCommittee mgapId committee = do
  ...
  where
    ...
    insertNewMember committeeId (cred, e) = do
      ...
      void . lift . DB.insertCommitteeMember $
        DB.CommitteeMember
...
resolveGovActionProposal syncEnv gaId = do
  ...
  liftFail (mkSyncNodeCallStack "resolveGovActionProposal") $ DB.queryGovActionProposalId gaTxId (fromIntegral index) -- TODO: Use Word32?
