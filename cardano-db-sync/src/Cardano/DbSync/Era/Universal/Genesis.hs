...
insertTxOuts syncEnv hasConsumed disInOut blkId (TxIn txInId _, txOut) = do
  ...
  _ <- insertStakeAddressRefIfMissing syncEnv UninitiatedCache (txOut ^. Core.addrTxOutL)
...
insertStaking syncEnv blkId genesis = do
  ...
  forM_ params $ uncurry (insertPoolRegister syncEnv UninitiatedCache (const False) network 0 blkId txId)
