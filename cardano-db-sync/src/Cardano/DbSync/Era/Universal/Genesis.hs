...
module Cardano.DbSync.Era.Universal.Genesis (
        ...
        Left _ ->
          runExceptT $ do
            ...
            when (hasInitialFunds || hasStakes) $ do
              ...
              lift $ mapM_ (insertTxOuts syncEnv hasConsumed disInOut bid) $ genesisUtxOs cfg
...
insertTxOuts syncEnv hasConsumed disInOut blkId (TxIn txInId _, txOut) = do
  ...
  _ <- insertStakeAddressRefIfMissing syncEnv useNoCache (txOut ^. Core.addrTxOutL)
...
insertStaking syncEnv blkId genesis = do
  ...
  forM_ params $ uncurry (insertPoolRegister syncEnv useNoCache (const False) Nothing network (EpochNo 0) blkId txId)
