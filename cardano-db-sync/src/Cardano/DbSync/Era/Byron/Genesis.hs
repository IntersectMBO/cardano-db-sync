      ...
      case ebid of
        ...
        Left err -> do
          ...
          when (not disInOut && count > 0) $
            throwError $
              DB.DbError DB.mkCallSite ("insertValidateGenesisDist: Genesis data mismatch. " <> show err) Nothing
          void $
            DB.insertMeta $
              DB.Meta
...
validateGenesisDistribution syncEnv prunes disInOut tracer networkName cfg bid = do
  ...
  unless disInOut $ do
    ...
    case DB.word64ToAda <$> configGenesisSupply cfg of
      Left err -> throwError $ DB.DbError DB.mkCallSite (Text.concat ["validateGenesisDistribution: ", textShow err]) Nothing
...
insertTxOutsByron syncEnv disInOut blkId (address, value) = do
  case txHashOfAddress address of
    Left err -> throwError $ DB.DbError DB.mkCallSite (Text.concat ["insertTxOutsByron: txHashOfAddress: ", show err]) Nothing
    Right val -> do
