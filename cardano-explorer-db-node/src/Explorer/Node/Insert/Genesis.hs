...
insertGenesisDistribution tracer cfg = do
  ...
  where
    ...
    insertAction = do
      ...
      bid <- either identity identity
                <$> DB.insertBlock (DB.Block genesisHash Nothing 0 Nothing Nothing 0)

      -- Insert an 'artificial' transaction for this block. TODO Is this this best hash???
      txid <- either identity identity <$> DB.insertTx (DB.Tx genesisHash bid 0)

      zipWithM_ (insertTxOuts txid) [0 .. ] (genesisTxos cfg)
...
insertTxOuts :: MonadIO m => DB.TxId -> Word16 -> (Ledger.Address, Ledger.Lovelace) -> ReaderT SqlBackend m ()
insertTxOuts txid index (address, value) = do
  void . DB.insertTxOut $
        DB.TxOut txid index (unAddressHash (Ledger.addrRoot address) <> "\0\0\0\0") (Ledger.unsafeGetLovelace value)
