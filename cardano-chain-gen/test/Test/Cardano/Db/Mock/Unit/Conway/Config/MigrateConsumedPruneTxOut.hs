...
migrateConsumeConfig :: SyncNodeConfig -> SyncNodeConfig
migrateConsumeConfig cfg = do
  cfg {dncInsertOptions = (dncInsertOptions cfg) {sioTxOut = TxOutConsumed (ForceTxIn False)}}

migratePruneConfigForceTxIn :: SyncNodeConfig -> SyncNodeConfig
migratePruneConfigForceTxIn cfg = do
  cfg {dncInsertOptions = (dncInsertOptions cfg) {sioTxOut = TxOutPrune (ForceTxIn True)}}

migratePruneConfig :: SyncNodeConfig -> SyncNodeConfig
migratePruneConfig cfg = do
  cfg {dncInsertOptions = (dncInsertOptions cfg) {sioTxOut = TxOutPrune (ForceTxIn False)}}

migrateBootstrapConfig :: SyncNodeConfig -> SyncNodeConfig
migrateBootstrapConfig cfg = do
...
txConsumedColumnCheck ioManager metadata = do
  syncNodeConfig <- mkCustomSyncNodeConfig conwayConfigDir args migrateConsumeConfig
  let syncNodeConfig' = syncNodeConfig {dncEnableLogging = True}
  withCustomConfig args (Just syncNodeConfig') conwayConfigDir testLabel action ioManager metadata
  where
...
basicPrune ioManager metadata = do
  syncNodeConfig <- mkCustomSyncNodeConfig conwayConfigDir args migratePruneConfigForceTxIn
  withCustomConfig args (Just syncNodeConfig) conwayConfigDir testLabel action ioManager metadata
  where
    action interpreter mockServer dbSync = do
      startDBSync dbSync
...
pruningShouldKeepSomeTx ioManager metadata = do
  syncNodeConfig <- mkCustomSyncNodeConfig conwayConfigDir args migratePruneConfig
  withCustomConfig args (Just syncNodeConfig) conwayConfigDir testLabel action ioManager metadata
  where
    action interpreter mockServer dbSync = do
      startDBSync dbSync
...
bootstrapRestartMissingFlag ioManager metadata = do
  syncNodeConfig <- mkCustomSyncNodeConfig conwayConfigDir args migrateBootstrapConfig
  withCustomConfig args (Just syncNodeConfig) conwayConfigDir testLabel action ioManager metadata
  where
    action interpreter mockServer dbSync = do
      startDBSync dbSync
