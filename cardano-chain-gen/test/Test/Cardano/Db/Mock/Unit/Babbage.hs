...
_mirRewardRollback =
    withFullConfig babbageConfig testLabel $ \interpreter mockServer dbSync -> do
      ...
      -- mir from treasury
      void $ withBabbageFindLeaderAndSubmitTx interpreter mockServer $
        Babbage.mkSimpleDCertTx [(StakeIndexNew 1,
...
deregistrationsScriptTx =
    ...
    void $ withBabbageFindLeaderAndSubmit interpreter mockServer $ \st -> do
      tx0 <- Babbage.mkSimpleDCertTx [(StakeIndexScript True, DCertDeleg . RegKey)] st
      tx1 <- Babbage.mkScriptDCertTx [ (StakeIndexScript True, True, DCertDeleg . DeRegKey)
...
deregistrationsScriptTx' =
    ...
    void $ withBabbageFindLeaderAndSubmit interpreter mockServer $ \st -> do
      tx0 <- Babbage.mkSimpleDCertTx [(StakeIndexScript True, DCertDeleg . RegKey)] st
      tx1 <- Babbage.mkScriptDCertTx [ (StakeIndexScript True, False, DCertDeleg . DeRegKey)
                               , (StakeIndexScript True, False, DCertDeleg .   RegKey)
                               , (StakeIndexScript True, True, DCertDeleg . DeRegKey)]
                               True st
      pure [tx0, tx1]
    ...
    withFullConfig babbageConfig testLabel $ \interpreter mockServer dbSync -> do
      ...
      txs' <- withBabbageLedgerState interpreter $ \st -> do
        ...
        tx1 <- Babbage.mkMAssetsScriptTx [UTxOIndex 0] (UTxOIndex 1)
          [(UTxOAddressNew 0, MaryValue 10000 mempty)] [UTxOPair utxo0] val0 True 100 st
        pure [tx0, tx1]
      void $ forgeNextFindLeaderAndSubmit interpreter mockServer (TxBabbage <$> txs')
