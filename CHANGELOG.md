...
- Document the `http-get-json-metadata` and `test-http-get-json-metadata` tools and correct the `http-get-json-metadata` usage output [#1729](https://github.com/IntersectMBO/cardano-db-sync/issues/1729).
...
- `cardano-db-tool validate` now runs every check and reports each result instead of aborting on the first failure, and exits non-zero if any check failed. Lookup errors (e.g. `utxo-set` on a missing slot) print just the message; infrastructure errors print the message plus a readable call chain instead of dumping the raw Haskell `DbCallStack` / `SrcLoc` records [#2163](https://github.com/IntersectMBO/cardano-db-sync/issues/2163).
- Fix `cardano-db-tool report` table alignment (columns now sized to the data), render timestamps at a uniform second precision, and correct help-text typos [#2164](https://github.com/IntersectMBO/cardano-db-sync/issues/2164).
- Fix two `cardano-db-tool validate` check bugs: the epoch block-tx check counted transactions by `block_no` instead of `block.id` (false mismatches on any real database), and the block-property checks underflowed their sampled block range on databases with fewer than 100000 blocks [#2170](https://github.com/IntersectMBO/cardano-db-sync/issues/2170).
